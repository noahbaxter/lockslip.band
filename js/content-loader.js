...
// Merch Carousel Functions
window.currentCollectionIndex = 0;

window.navigateItemCarousel = function(itemId, direction) {
...
window.goToItemImage = function(itemId, index) {
    const carousel = document.querySelector(`[data-item-id="${itemId}"] .merch-image-carousel`);
    if (!carousel) return;
...
window.navigateCollectionCarousel = function(direction) {
    ...
    window.currentCollectionIndex += direction;
    
    if (shouldLoop) {
        // Infinite looping for carousels with enough items
        if (window.currentCollectionIndex < 0) {
            window.currentCollectionIndex = maxIndex;
        }
        if (window.currentCollectionIndex > maxIndex) {
            window.currentCollectionIndex = 0;
        }
    } else {
    ...
    const translatePercent = window.currentCollectionIndex * stepPercent;
    const translateGapRem = window.currentCollectionIndex * stepGapRem;
    ...
    if (shouldLoop) {
        ...
        if (prevBtn) prevBtn.style.display = window.currentCollectionIndex > 0 ? 'flex' : 'none';
        if (nextBtn) nextBtn.style.display = window.currentCollectionIndex < maxIndex ? 'flex' : 'none';
    }
};